...
* The modules `Category.Functor` and `Category.Applicative.Index` do not
  export the record `Morphism`, due to a clash with the new definition of
  the type `Morphism A B` in `Function.Core`. Instead, `Category.Functor`
  now exports the record `CategoryMorphism` and `Category.Applicative.Index`
  exports the record `ApplicativeMorphism`.

...
* Added the function `lift₀` to `Data.Fin.Permutation` which takes a
  permutation `Fin n → Fin m` to a permutation `Fin (suc n) → Fin (suc
  m)` by mapping `0` to `0` and applying the input permutation to
  everything else.

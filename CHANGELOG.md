...
* List-literal syntax for `List`/`Vec`/`Vector`, allowing one to write 
...
### Type-class providing list literal-like syntax

* Added a new type-class in `Data.TypeClasses.ListSyntax` that allows
  one to write `[ x , y , z ]` when constructing an element of the type.

* This replaces the existing `[_]` singleton operators for `List` and `Vec`.
  This should be backwards compatible in all cases where `[_]` was applied
  to an argument. However where `[_]` was used as a unapplied function,
  it will have to be replaced with the function `_∷ []`.
  
* Currently the type-class is implemented by `List`, `Vec` and `Vector`.

* The `[ x , y , z ]` syntax cannot be used in pattern matching.

...
import Data.Vec.Functional.Properties as Vectorₚ
...
module _ {n} where
  open B.Setoid (Pw.setoid setoid n)  public
    using ()
    renaming (_≈_ to _≋_)
...
sumₜ-cong-≈ {suc n} p = +-cong (p zero) (sumₜ-cong-≈ (p ∘ suc))
...
∑-comm : ∀ n m (f : Fin n → Fin m → Carrier) →
         ∑[ i < n ] ∑[ j < m ] f i j ≈ ∑[ j < m ] ∑[ i < n ] f i j
...
sumₜ-permute {suc m} {suc n} t π = begin
  sumₜ t
    ≡⟨⟩
  t 0i           + sumₜ (remove 0i t)
    ≡⟨ P.cong₂ _+_ (P.cong t (P.sym (Perm.inverseʳ π))) P.refl ⟩
  πt (π ⟨$⟩ˡ 0i) + sumₜ (remove 0i t)
    ≈⟨ +-congˡ (sumₜ-permute (remove 0i t) (Perm.remove (π ⟨$⟩ˡ 0i) π)) ⟩
  πt (π ⟨$⟩ˡ 0i) + sumₜ (permute (Perm.remove (π ⟨$⟩ˡ 0i) π) (remove 0i t))
    ≡⟨ P.cong₂ _+_ P.refl (sumₜ-cong-≡ (P.sym ∘ Vectorₚ.remove-permute π 0i t)) ⟩
  πt (π ⟨$⟩ˡ 0i) + sumₜ (remove (π ⟨$⟩ˡ 0i) πt)
    ≈⟨ sym (sumₜ-remove (permute π t)) ⟩
  sumₜ πt
    ∎
  where
  0i = zero
...
select-transpose :
  ...
  ∀ k → (select 0# j t ∘ PermC.transpose i j) k
      ≈  select 0# i t k
select-transpose _ i j e k with k Finₚ.≟ i
... | true  because _ rewrite dec-true (j Finₚ.≟ j) P.refl = sym e
... | false because [k≢i] with k Finₚ.≟ j
...   | true  because [k≡j]
  rewrite dec-false (i Finₚ.≟ j) (invert [k≢i] ∘ P.trans (invert [k≡j]) ∘ P.sym)
        = refl
...   | false because [k≢j] rewrite dec-false (k Finₚ.≟ j) (invert [k≢j]) = refl
...
sumₜ-select {suc n} {i} t = begin
  sumₜ (select 0# i t)
    ≈⟨ sumₜ-remove {i = i} (select 0# i t) ⟩
  select 0# i t i + sumₜ (remove i (select 0# i t))
    ≡⟨ P.cong₂ _+_ (Vectorₚ.select-lookup t) (sumₜ-cong-≡ (Vectorₚ.select-remove i t)) ⟩
  t i + sumₜ (replicate {n = n} 0#)
    ≈⟨ +-congˡ (sumₜ-zero n) ⟩
  t i + 0#
    ≈⟨ +-identityʳ _ ⟩
  t i

...
isGCD[0,x,x] x =  gcdᶜ (_∣0 x) ∣-refl (λ _ y∣x → y∣x)

isGCD[x,0,x] : ∀ x → IsGCD x 0# x
isGCD[x,0,x] x =  gcdᶜ ∣-refl (_∣0 x) (λ y∣x _ → y∣x)
...
x≈0∧y≈0⇒gcd≈0 : ∀ {x y d} → IsGCD x y d → x ≈ 0# → y ≈ 0# → d ≈ 0#
x≈0∧y≈0⇒gcd≈0 (gcdᶜ _ _ greatest) x≈0 y≈0 =  0∣x⇒x≈0 0∣d
  where
...
coprime⇒gcd∣1 coprime[x,y] (gcdᶜ divides₁ divides₂ _) = coprime[x,y] divides₁ divides₂

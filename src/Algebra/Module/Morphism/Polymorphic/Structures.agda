...
--
-- Morphisms between module-like algebraic structures over different ring structures
------------------------------------------------------------------------
{-# OPTIONS --cubical-compatible --safe #-}

module Algebra.Module.Morphism.Polymorphic.Structures where
...
private
  variable
    r s rl rr sl sr m₁ ℓm₁ m₂ ℓm₂ : Level

module LeftSemimoduleMorphisms
  {R : Set r} {S : Set s}
  ([_] : R → S)
  (M₁ : RawLeftSemimodule R m₁ ℓm₁)
  (M₂ : RawLeftSemimodule S m₂ ℓm₂)
  where
  ...
  open MorphismDefinitions R S [_] A B _≈ᴹ₂_
...
module LeftModuleMorphisms
  ...
  open LeftSemimoduleMorphisms [_] (RawLeftModule.rawLeftSemimodule M₁) (RawLeftModule.rawLeftSemimodule M₂)
...
module RightModuleMorphisms
  ...
  open RightSemimoduleMorphisms [_] (RawRightModule.rawRightSemimodule M₁) (RawRightModule.rawRightSemimodule M₂)
...
module BisemimoduleMorphisms
  {RL : Set rl} {SL : Set sl} ([_]ₗ : RL → SL)
  {RR : Set rr} {SR : Set sr} ([_]ᵣ : RR → SR)
  (M₁ : RawBisemimodule RL RR m₁ ℓm₁)
  (M₂ : RawBisemimodule SL SR m₂ ℓm₂)
  where
  ...
  open MorphismDefinitions RL SL [_]ₗ A B _≈ᴹ₂_ using (Homomorphicₗ)
  open MorphismDefinitions RR SR [_]ᵣ A B _≈ᴹ₂_ using (Homomorphicᵣ)
  open MorphismStructures.MonoidMorphisms (RawBisemimodule.+ᴹ-rawMonoid M₁) (RawBisemimodule.+ᴹ-rawMonoid M₂)
  open LeftSemimoduleMorphisms [_]ₗ (RawBisemimodule.rawLeftSemimodule M₁) (RawBisemimodule.rawLeftSemimodule M₂)
  open RightSemimoduleMorphisms [_]ᵣ (RawBisemimodule.rawRightSemimodule M₁) (RawBisemimodule.rawRightSemimodule M₂)

  record IsBisemimoduleHomomorphism (⟦_⟧ : A → B) : Set (rl ⊔ rr ⊔ m₁ ⊔ ℓm₁ ⊔ ℓm₂) where
    field
      +ᴹ-isMonoidHomomorphism : IsMonoidHomomorphism ⟦_⟧
      *ₗ-homo                 : Homomorphicₗ ⟦_⟧ _*ₗ₁_ _*ₗ₂_
      *ᵣ-homo                 : Homomorphicᵣ ⟦_⟧ _*ᵣ₁_ _*ᵣ₂_
  ...
  record IsBisemimoduleMonomorphism (⟦_⟧ : A → B) : Set (rl ⊔ rr ⊔ m₁ ⊔ ℓm₁ ⊔ ℓm₂) where
    field
  ...
  record IsBisemimoduleIsomorphism (⟦_⟧ : A → B) : Set (rl ⊔ rr ⊔ m₁ ⊔ m₂ ⊔ ℓm₁ ⊔ ℓm₂) where
    field
...
module BimoduleMorphisms
  {RL : Set rl} {SL : Set sl} ([_]ₗ : RL → SL)
  {RR : Set rr} {SR : Set sr} ([_]ᵣ : RR → SR)
  (M₁ : RawBimodule RL RR m₁ ℓm₁)
  (M₂ : RawBimodule SL SR m₂ ℓm₂)
  where
  ...
  open LeftModuleMorphisms [_]ₗ (RawBimodule.rawLeftModule M₁) (RawBimodule.rawLeftModule M₂)
  open RightModuleMorphisms [_]ᵣ (RawBimodule.rawRightModule M₁) (RawBimodule.rawRightModule M₂)
  open BisemimoduleMorphisms [_]ₗ [_]ᵣ (RawBimodule.rawBisemimodule M₁) (RawBimodule.rawBisemimodule M₂)

  record IsBimoduleHomomorphism (⟦_⟧ : A → B) : Set (rl ⊔ rr ⊔ m₁ ⊔ ℓm₁ ⊔ ℓm₂) where
    field
      +ᴹ-isGroupHomomorphism : IsGroupHomomorphism ⟦_⟧
      *ₗ-homo                 : Homomorphicₗ ⟦_⟧ _*ₗ₁_ _*ₗ₂_
      *ᵣ-homo                 : Homomorphicᵣ ⟦_⟧ _*ᵣ₁_ _*ᵣ₂_
  ...
  record IsBimoduleMonomorphism (⟦_⟧ : A → B) : Set (rl ⊔ rr ⊔ m₁ ⊔ ℓm₁ ⊔ ℓm₂) where
    field
  ...
  record IsBimoduleIsomorphism (⟦_⟧ : A → B) : Set (rl ⊔ rr ⊔ m₁ ⊔ m₂ ⊔ ℓm₁ ⊔ ℓm₂) where
    field
...
module SemimoduleMorphisms
  {R : Set r} {S : Set s} ([_] : R → S)
  (M₁ : RawSemimodule R m₁ ℓm₁)
  (M₂ : RawSemimodule S m₂ ℓm₂)
  where

  open RawSemimodule M₁ renaming (Carrierᴹ to A; _≈ᴹ_ to _≈ᴹ₁_)
  open RawSemimodule M₂ renaming (Carrierᴹ to B; _≈ᴹ_ to _≈ᴹ₂_)
  open BisemimoduleMorphisms [_] [_] (RawSemimodule.rawBisemimodule M₁) (RawSemimodule.rawBisemimodule M₂)
...
module ModuleMorphisms
  ...
  open BimoduleMorphisms [_] [_] (RawModule.rawBimodule M₁) (RawModule.rawBimodule M₂)
  open SemimoduleMorphisms [_] (RawModule.rawBisemimodule M₁) (RawModule.rawBisemimodule M₂)

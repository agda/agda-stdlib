...
module _ (commutativeMonoid : CommutativeMonoid a r) where
  ...
  foldr-commMonoid : ∀ {xs ys} → xs ↭ ys → List.foldr _∙_ ε xs ≈ List.foldr _∙_ ε ys
  foldr-commMonoid refl = CM.refl
  foldr-commMonoid (prep x xs↭ys) = ∙-congˡ (foldr-commMonoid xs↭ys)
  foldr-commMonoid (swap {xs} {ys} x y xs↭ys) = begin
...
product-↭ = foldr-commMonoid *-1-commutativeMonoid

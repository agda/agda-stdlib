...
open import Data.Vec.Functional.Relation.Binary.Pointwise.Properties using (_≗_; ≡-setoid)
...
fromList-∈ : ∀ {xs : List A} (i : Fin (L.length xs)) → fromList xs i ∈ xs
...
fromList-index : ∀ {xs} {x : A} (x∈xs : x ∈ xs) → fromList xs (index x∈xs) ≡ x
...
↔Vec = record
  ...
  ; from = P.→-to-⟶ fromVec
  ; inverse-of = record
    { left-inverse-of  = VP.lookup∘tabulate

...
-- A backwards list (also as snoc list because `snoc' is `cons' backwards)
-- is a list that grows on the right, and that is therefore traversed
-- right-to-left.
...
-- can move the position around either left or right by moving values
...
--
-- This can be done efficiently if my representation on the left is
-- right-to-left and so gives me constant time access to its right end

...
open import Data.Nat using (ℕ; s≤s)
  renaming (suc to 1+_; pred to predₙ; _+_ to _+ₙ_; _*_ to _*ₙ_; _∸_ to _∸ₙ_; _≤_ to _≤ₙ_)
...
import Algebra.Properties.CommutativeSemigroup ℕₚ.+-semigroup ℕₚ.+-comm as Of+ℕ-semigroup
import Algebra.Properties.CommutativeSemigroup ℕₚ.*-semigroup ℕₚ.*-comm as Of*ℕ-semigroup
...
toℕ∘2* (2suc  x) = cong (2 *ₙ_) (sym (ℕₚ.*-distribˡ-+ 2 1 (toℕ x)))
...
toℕ∘pred 0#        =  refl
toℕ∘pred (2suc x)  =  cong predₙ $ sym $ ℕₚ.*-distribˡ-+ 2 1 (toℕ x)
...
toℕ-homo-+ 0#        _        = refl
toℕ-homo-+ (2suc  x) 0#       = cong 1+_ (sym (ℕₚ.+-identityʳ _))
toℕ-homo-+ (suc2* x) 0#       = cong 1+_ (sym (ℕₚ.+-identityʳ _))
toℕ-homo-+ (2suc x)  (2suc y) = begin
  toℕ ((2suc x) + (2suc y))        ≡⟨⟩
  toℕ (2suc (suc (x + y)))         ≡⟨⟩
  2 *ₙ (1 +ₙ (toℕ (suc (x + y))))  ≡⟨ cong ((2 *ₙ_) ∘ 1+_) (toℕ∘suc (x + y)) ⟩
  2 *ₙ (2 +ₙ toℕ (x + y))          ≡⟨ cong ((2 *ₙ_) ∘ (2 +ₙ_)) (toℕ-homo-+ x y) ⟩
  2 *ₙ (2 +ₙ (m +ₙ n))             ≡⟨ cong ((2 *ₙ_) ∘ 1+_)
                                                    (sym (ℕₚ.+-assoc 1 m n)) ⟩
  ...
  2 *ₙ ((1+ ((m +ₙ (1+ n)))))      ≡⟨ cong (2 *ₙ_)
                                           (sym (ℕₚ.+-assoc 1 m (1+ n)))
                                    ⟩
  2 *ₙ ((1+ m) +ₙ (1+ n))          ≡⟨ ℕₚ.*-distribˡ-+ 2 (1+ m) (1+ n) ⟩
  (2 *ₙ (1+ m)) +ₙ (2 *ₙ (1+ n))   ≡⟨⟩
...
toℕ-homo-+ (2suc x) (suc2* y) = begin
  ...
  1+ (2 *ₙ (1+ (m +ₙ n)))         ≡⟨ cong (1+_ ∘ (2 *ₙ_))
                                                 (sym (ℕₚ.+-assoc 1 m n)) ⟩
  1+ (2 *ₙ (1+m +ₙ n))            ≡⟨ cong 1+_ (ℕₚ.*-distribˡ-+ 2 1+m n) ⟩
...
toℕ-homo-+ (suc2* x) (2suc y) = begin
  ...
  1+ (2 *ₙ (1+ (m +ₙ n)))         ≡⟨ cong (1+_ ∘ (2 *ₙ_))
                                   ...
                                   ⟩
  1+ (2 *ₙ (m +ₙ 1+n))            ≡⟨ cong 1+_ (ℕₚ.*-distribˡ-+ 2 m 1+n) ⟩
  1+ (2 *ₙ m +ₙ 2 *ₙ 1+n)         ≡⟨ sym (ℕₚ.+-assoc 1 (2 *ₙ m) (2 *ₙ 1+n)) ⟩
  (1+ (2 *ₙ m)) +ₙ (2 *ₙ (1+ n))  ≡⟨⟩
...
toℕ-homo-+ (suc2* x) (suc2* y) = begin
  ...
  1+ (toℕ (suc2* (x + y)))          ≡⟨⟩
  1+ (1+ (2 *ₙ (toℕ (x + y))))      ≡⟨ cong (1+_ ∘ 1+_ ∘ (2 *ₙ_)) (toℕ-homo-+ x y) ⟩
  1+ (1+ (2 *ₙ (m +ₙ n)))           ≡⟨ cong (1+_ ∘ 1+_) (ℕₚ.*-distribˡ-+ 2 m n) ⟩
  1+ (1+ (2 *ₙ m +ₙ 2 *ₙ n))        ≡⟨ cong 1+_ (sym (ℕₚ.+-assoc 1 (2 *ₙ m) _)) ⟩
  1+ ((1+ (2 *ₙ m) +ₙ (2 *ₙ n)))    ≡⟨ (Of+ℕ-semigroup.x∙yz≈y∙xz 1 (1+ (2 *ₙ m)) _) ⟩
  (1+ (2 *ₙ m)) +ₙ (1+ (2 *ₙ n))    ≡⟨⟩
...
+-assoc a b c = begin
  ...
  fromℕ ((aN +ₙ bN) +ₙ cN)   ≡⟨ cong fromℕ (ℕₚ.+-assoc aN bN cN) ⟩
  fromℕ (aN +ₙ (bN +ₙ cN))   ≡⟨ cong (fromℕ ∘ (aN +ₙ_)) (sym (toℕ-homo-+ b c)) ⟩
...
+-comm a b = begin
  ...
  fromℕ (toℕ a +ₙ toℕ b)   ≡⟨ cong fromℕ (ℕₚ.+-comm (toℕ a) (toℕ b)) ⟩
...
+-cancelˡ-≡ x {y} {z} x+y≡x+z = begin
  ...
  m≡n = begin
    ...
    (m +ₙ k) ∸ₙ k    ≡⟨ cong (_∸ₙ k) (ℕₚ.+-comm m k) ⟩
    (k +ₙ m) ∸ₙ k    ≡⟨ cong (_∸ₙ k) eq ⟩
    (k +ₙ n) ∸ₙ k    ≡⟨ cong (_∸ₙ k) (ℕₚ.+-comm k n) ⟩
    (n +ₙ k) ∸ₙ k    ≡⟨ ℕₚ.m+n∸n≡m n k ⟩
    n                ∎
...
toℕ-homo-* :  ∀ x y → toℕ (x * y) ≡ toℕ x *ₙ toℕ y
toℕ-homo-* x y = aux x y (size x +ₙ size y) ℕₚ.≤-refl
  where
  aux :  (x y : Bin) → (cnt : ℕ) → (size x +ₙ size y ≤ₙ cnt) →
         toℕ (x * y) ≡ toℕ x *ₙ toℕ y

  aux 0#        _  _ _ = refl
  aux (2suc x)  0# _ _ = sym (ℕₚ.*-zeroʳ (toℕ x +ₙ (1+ (toℕ x +ₙ 0))))
  aux (suc2* x) 0# _ _ = sym (ℕₚ.*-zeroʳ (toℕ x +ₙ (toℕ x +ₙ 0)))
  aux (2suc x) (2suc y) (1+ cnt) (s≤s |x|+1+|y|≤cnt) = begin
    ...
    2 *ₙ (2 *ₙ (1+m *ₙ 1+n))       ≡⟨ cong (2 *ₙ_) (Of*ℕ-semigroup.x∙yz≈y∙xz
                                                                 2 1+m 1+n) ⟩
    2 *ₙ (1+m *ₙ (2 *ₙ 1+n))       ≡⟨ sym (ℕₚ.*-assoc 2 1+m (2 *ₙ 1+n)) ⟩
    (2 *ₙ 1+m) *ₙ (2 *ₙ 1+n)       ≡⟨⟩
    toℕ (2suc x) *ₙ toℕ (2suc y)   ∎
    where
    m   = toℕ x;   n  = toℕ y;   1+m = 1+ m
    ...
    eq  = begin
      (1+m *ₙ (1+ n))           ≡⟨ ℕₚ.*-distribˡ-+ 1+m 1 n ⟩
      (1+m *ₙ 1 +ₙ 1+m *ₙ n)    ≡⟨ cong (_+ₙ (1+m *ₙ n)) (ℕₚ.*-identityʳ 1+m) ⟩
    ...
    |x|+|y|≤cnt = ℕₚ.≤-trans (ℕₚ.+-monoʳ-≤ (size x) (ℕₚ.n≤1+n (size y)))
                              |x|+1+|y|≤cnt
  ...
  aux (2suc x) (suc2* y) (1+ cnt) (s≤s |x|+1+|y|≤cnt) = begin
    ...
    2 *ₙ (1+m +ₙ (n *ₙ 2[1+m]))          ≡⟨ ℕₚ.*-distribˡ-+ 2 1+m _ ⟩
    2[1+m] +ₙ (2 *ₙ (n *ₙ 2[1+m]))       ≡⟨ cong (2[1+m] +ₙ_) $
                                            sym (ℕₚ.*-assoc 2 n _) ⟩
    2[1+m] +ₙ (2 *ₙ n) *ₙ 2[1+m]         ≡⟨ cong (_+ₙ ((2 *ₙ n) *ₙ 2[1+m]))
                                              (sym (ℕₚ.*-identityˡ 2[1+m])) ⟩
    1 *ₙ 2[1+m] +ₙ 2n *ₙ 2[1+m]          ≡⟨ sym (ℕₚ.*-distribʳ-+ 2[1+m]
                                                                   1 2n) ⟩
    (1+ 2n) *ₙ 2[1+m]                    ≡⟨ ℕₚ.*-comm (1+ 2n) 2[1+m] ⟩
    2[1+m] *ₙ (1+ 2n)                    ≡⟨⟩
    toℕ (2suc x) *ₙ toℕ (suc2* y)        ∎
    where
    m      = toℕ x;       n  = toℕ y;   1+m = 1+ m
  ...
  aux (suc2* x) (2suc y) (1+ cnt) (s≤s |x|+1+|y|≤cnt) = begin
    ...
    2 *ₙ (1+n +ₙ (m *ₙ 2[1+n]))        ≡⟨ ℕₚ.*-distribˡ-+ 2 1+n _ ⟩
    2[1+n] +ₙ (2 *ₙ (m *ₙ 2[1+n]))     ≡⟨ cong (2[1+n] +ₙ_)
                                               (sym (ℕₚ.*-assoc 2 m _)) ⟩
    2[1+n] +ₙ 2m *ₙ 2[1+n]             ≡⟨ cong (_+ₙ (2m *ₙ 2[1+n]))
                                               (sym (ℕₚ.*-identityˡ 2[1+n])) ⟩
    1 *ₙ 2[1+n] +ₙ 2m *ₙ 2[1+n]        ≡⟨ sym (ℕₚ.*-distribʳ-+ 2[1+n] 1 2m) ⟩
    (1+ 2m) *ₙ 2[1+n]                  ≡⟨⟩
    toℕ (suc2* x) *ₙ toℕ (2suc y)      ∎
    where
    m  = toℕ x;   n      = toℕ y;      1+n = 1+ n
  ...
  aux (suc2* x) (suc2* y) (1+ cnt) (s≤s |x|+1+|y|≤cnt) = begin
    ...
    [1+2x]' +ₙ 2n *ₙ [1+2x]'           ≡⟨ cong (_+ₙ (2n *ₙ [1+2x]')) $
                                        ...
                                        ⟩
    1 *ₙ [1+2x]' +ₙ 2n *ₙ [1+2x]'      ≡⟨ sym (ℕₚ.*-distribʳ-+ [1+2x]' 1 2n) ⟩
    (1+ 2n) *ₙ [1+2x]'                 ≡⟨ ℕₚ.*-comm (1+ 2n) [1+2x]' ⟩
    toℕ (suc2* x) *ₙ toℕ (suc2* y)     ∎
    where
...
*-identityˡ x = begin
  ...
  fromℕ (1 *ₙ n)         ≡⟨ cong fromℕ (ℕₚ.*-identityˡ n) ⟩
  fromℕ n                ≡⟨ fromℕ∘toℕ x ⟩
  x                      ∎
  where
  n = toℕ x
...
*-distribˡ-+ a b c = begin
  a * (b + c)                          ≡⟨ sym (fromℕ∘toℕ (a * (b + c))) ⟩
  fromℕ (toℕ (a * (b + c)))            ≡⟨ cong fromℕ (toℕ-homo-* a (b + c)) ⟩
  fromℕ (k *ₙ (toℕ (b + c)))           ≡⟨ cong (fromℕ ∘ (k *ₙ_)) (toℕ-homo-+ b c) ⟩
  fromℕ (k *ₙ (m +ₙ n))                ≡⟨ cong fromℕ (ℕₚ.*-distribˡ-+ k m n) ⟩
  fromℕ (k *ₙ m +ₙ k *ₙ n)             ≡⟨ cong fromℕ (sym (
                                            cong₂ _+ₙ_ (toℕ-homo-* a b) (toℕ-homo-* a c))) ⟩
  fromℕ (toℕ (a * b) +ₙ toℕ (a * c))   ≡⟨ cong fromℕ (sym (toℕ-homo-+ (a * b) (a * c))) ⟩

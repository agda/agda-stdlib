...
open import Data.Bin.Base using (Bin; _≟_; suc; pred; _+_; _*_; 2*; 2^; size;
                                                                1#; 2#; toℕ)
...
open import Data.Nat.Properties as ℕp using (2m≢1+2n; module ≤-Reasoning)
open import Data.Product    using (_,_; proj₁; ∃)
open import Data.Sum        using (_⊎_; inj₁; inj₂)
open import Function        using (_∘_; _$_; id; const; case_of_)
open import Level           using () renaming (zero to 0ℓ)
open import Relation.Binary using (Setoid; DecSetoid; IsDecEquivalence)
...
import Algebra.Properties.CommutativeSemigroup ℕp.*-semigroup ℕp.*-comm
                                                     as Of*ℕ-semigroup
open FuncProp (_≡_ {A = Bin}) using
              (Associative; Commutative; LeftIdentity; RightIdentity;
               LeftZero; RightZero; LeftCancellative; RightCancellative;
               _DistributesOverˡ_; _DistributesOverʳ_; _DistributesOver_)
...
toℕ∘pred :  (x : Bin) → toℕ (pred x) ≡ predₙ (toℕ x)
toℕ∘pred x with x ≟ 0#
... | yes x≡0 = begin
  toℕ (pred x)   ≡⟨ cong (toℕ ∘ pred) x≡0 ⟩
  0               ≡⟨ sym (cong (predₙ ∘ toℕ) x≡0) ⟩
  predₙ (toℕ x)  ∎
...
... | no x≢0 = begin
  ...
  toℕ px                 ≡⟨⟩
  predₙ (1+ (toℕ px))    ≡⟨ cong predₙ (sym (toℕ∘suc px)) ⟩
  predₙ (toℕ (suc px))   ≡⟨ cong (predₙ ∘ toℕ) suc-px≡x ⟩
  predₙ (toℕ x)          ∎
  where
...
toℕ+homo :  (x y : Bin) → toℕ (x + y) ≡ toℕ x +ₙ toℕ y
toℕ+homo 0# _  =  refl
toℕ+homo x  0# =  begin
...
toℕ+homo (2suc x) (2suc y) = begin
  ...
  2 *ₙ (1+ (1+ toℕ (x + y)))      ≡⟨ cong ((2 *ₙ_) ∘ 1+_ ∘ 1+_) (toℕ+homo x y) ⟩
  2 *ₙ ((1+ (1+ (m +ₙ n))))        ≡⟨ cong ((2 *ₙ_) ∘ 1+_)
                                                     (sym (ℕp.+-assoc 1 m n)) ⟩
  2 *ₙ ((1+ ((1+ m) +ₙ n)))        ≡⟨ cong ((2 *ₙ_) ∘ 1+_ ∘ (_+ₙ n))
                                                          (ℕp.+-comm 1 m) ⟩
  2 *ₙ ((1+ ((m +ₙ 1) +ₙ n)))      ≡⟨ cong ((2 *ₙ_) ∘ 1+_) (ℕp.+-assoc m 1 n) ⟩
  2 *ₙ ((1+ ((m +ₙ (1+ n)))))      ≡⟨ cong (2 *ₙ_) (sym (ℕp.+-assoc 1 m (1+ n)))
                                    ⟩
...
+-0-isMonoid = record{ isSemigroup = +-isSemigroup
                     ; identity    = (+-identityˡ , +-identityʳ) }

+-0-monoid : Monoid 0ℓ 0ℓ
+-0-monoid = record{ Carrier  = Bin
                   ; _≈_      = _≡_
                   ; _∙_      = _+_
                   ; ε        = 0#
                   ; isMonoid = +-0-isMonoid }
...
+-0-isCommutativeMonoid = record{ isSemigroup = +-isSemigroup
                                ; identityˡ   = +-identityˡ
                                ; comm        = +-comm }
...
------------------------------------------------------------------------------
-- Instances for Bin up to CommutativeRing
------------------------------------------------------------------------------
...
toℕ*homo x y =  aux x y (size x +ₙ size y) ℕp.≤-refl
  ...
  aux (2suc x) (2suc y) (1+ cnt) (s≤s |x|+1+|y|≤cnt) = begin
    ...
    |x|+|y|≤cnt = begin≤
      size x +ₙ size y          ≤[ ℕp.+-monoʳ-≤ (size x) (ℕp.n≤1+n (size y)) ]
      size x +ₙ (1+ (size y))   ≤[ |x|+1+|y|≤cnt ]
      cnt                       end≤
  ...
  aux (2suc x) (suc2* y) (1+ cnt) (s≤s |x|+1+|y|≤cnt) = begin
    ...
    toℕ (2suc x) *ₙ toℕ (suc2* y)       ∎
    where
    m  = toℕ x;   n = toℕ y;   1+m = 1+ m;   2[1+m] = 2 *ₙ (1+ m)
    2n = 2 *ₙ n

    |y|+1+|x|≤cnt = begin≤
      size y +ₙ (1+ (size x))    ≡≤[ Of+ℕ-semigroup.x∙yz≈y∙xz (size y) 1 _ ]
      (1+ (size y)) +ₙ size x    ≡≤[ ℕp.+-comm (1+ (size y)) (size x) ]
      size x +ₙ (1+ (size y))     ≤[ |x|+1+|y|≤cnt ]
      cnt                        end≤
  ...
  aux (suc2* x) (2suc y) (1+ cnt) (s≤s |x|+1+|y|≤cnt) = begin
    ...
    toℕ (suc2* x) *ₙ toℕ (2suc y)     ∎
    where
    m      = toℕ x;      n = toℕ y;    1+n = 1+ n;    2m = 2 *ₙ m
    2[1+n] = 2 *ₙ (1+ n)
  ...
  aux (suc2* x) (suc2* y) (1+ cnt) (s≤s |x|+1+|y|≤cnt) = begin
    ...
    |y|+1+|x|≤cnt = begin≤
      size y +ₙ (1+ (size x))     ≡≤[ ℕp.+-comm (size y) (1+ (size x)) ]
      (1+ (size x)) +ₙ size y     ≡≤[ Of+ℕ-semigroup.x∙yz≈y∙xz 1 (size x) _ ]
      size x +ₙ (1+ (size y))      ≤[ |x|+1+|y|≤cnt ]
      cnt                         end≤
...
2x≡0⇒x≡0 {x} 2x≡0 =  x≡0
  ...
  x≡0 :  x ≡ 0#
  x≡0 with  x*y≡0⇒x≡0∨y≡0 2# 2#*x≡0
...
2*-distrib x y = begin
  2* (x + y)            ≡⟨ 2*≗2#* (x + y) ⟩
  2# * (x + y)          ≡⟨ *-distribˡ-+ 2# x y ⟩
  (2# * x) + (2# * y)   ≡⟨ cong₂ _+_ eq eq' ⟩
  2* x + 2* y           ∎
  where
  eq = sym (2*≗2#* x);   eq' = sym (2*≗2#* y)


*-identityˡ :  (1# *_) ≗ id
*-identityˡ x = begin
  1# * x                   ≡⟨ sym (fromℕ∘toℕ (1# * x)) ⟩
...
*-identityʳ :  (_* 1#) ≗ id
*-identityʳ x =  trans (*-comm x 1#) (*-identityˡ x)

2#*x≡x+x :  (x : Bin) → 2# * x ≡ x + x
2#*x≡x+x x = begin
  2# * x            ≡⟨⟩
  (1# + 1#) * x     ≡⟨ *-distribʳ-+ x 1# 1# ⟩
  1# * x + 1# * x   ≡⟨ cong₂ _+_ (*-identityˡ x) (*-identityˡ x) ⟩
  x + x             ∎

2x≡x+x :  (x : Bin) → 2* x ≡ x + x
2x≡x+x x =  trans (2*≗2#* x) (2#*x≡x+x x)

[1+x]* :  (x y : Bin) → (1# + x) * y ≡ y + x * y
[1+x]* x y = begin
  (1# + x) * y     ≡⟨ *-distribʳ-+ y 1# x ⟩
  1# * y + x * y   ≡⟨ cong (_+ (x * y)) (*-identityˡ y) ⟩
  y + x * y        ∎

*[1+x] :  (x y : Bin) → y * (1# + x) ≡ y + y * x
*[1+x] x y = begin
  y * (1# + x)     ≡⟨ *-distribˡ-+ y 1# x ⟩
  y * 1# + y * x   ≡⟨ cong (_+ (y * x)) (*-identityʳ y) ⟩
  y + y * x        ∎
...
*-1-isMonoid = record{ isSemigroup = *-isSemigroup
                     ; identity    = (*-identityˡ , *-identityʳ) }

*-1-monoid : Monoid 0ℓ 0ℓ
*-1-monoid = record{ Carrier  = Bin
                   ; _≈_      = _≡_
                   ; _∙_      = _*_
                   ; ε        = 1#
                   ; isMonoid = *-1-isMonoid }
...
*-1-isCommutativeMonoid =  record{ isSemigroup = *-isSemigroup
                                 ; identityˡ    = *-identityˡ
                                 ; comm         = *-comm }

...
open import Algebra.FunctionProperties using (Op₂; Associative; Commutative)
...
open import Function.Properties using (Injective; Surjective)
...
2suc-injective : Injective _≡_ _≡_ 2suc
...
suc2*-injective : Injective _≡_ _≡_ suc2*
...
2suc-as∘ :  2suc ≗ 2* ∘ suc
2suc-as∘ 0#        =  refl
2suc-as∘ (2suc x)  =  begin 2suc (2suc x)        ≡⟨ cong 2suc (2suc-as∘ x) ⟩
                            2suc (2* (suc x))    ≡⟨⟩
                            2* (suc2* (suc x))   ≡⟨⟩
                            2* (suc (2suc x))
                      ∎
...
suc∘pred (suc2* x) _   =  begin suc (pred (suc2* x))   ≡⟨⟩
...
nz*nz x≢0 y≢0 xy≡0  with x*y≡0⇒⊎ xy≡0
...                 | inj₁ x≡0 =  x≢0 x≡0
...                 | inj₂ y≡0 =  y≢0 y≡0
...
... | no x≢0 =
  begin toℕ (pred x)           ≡⟨ cong (toℕ ∘ pred) (sym suc-px≡x) ⟩
        ...
        pred' (toℕ x)
  ∎
  where px = pred x;  suc-px≡x = suc∘pred x x≢0
...
fromℕ₁∘toℕ a =  let (b , toℕ-b≡toℕ-a) = fromℕ (toℕ a)
                 in

...
open import Data.Fin using () renaming (zero to 0F; suc to sucF)
...
open import Relation.Binary.PropositionalEquality as P using
  (_≡_; _≢_; _≗_; refl; sym; trans; cong; cong₂; resp₂; subst; module ≡-Reasoning)
...
private module FP-Bin = FuncProp (_≡_ {A = ℕᵇ})
open Tri
...
x>y⇒x∸y≢0 : ∀ {x y} → x > y → x ∸ y ≢ 0ᵇ
x>y⇒x∸y≢0 {zero} {_} ()
x>y⇒x∸y≢0 {x'@(2[1+ x ])} {zero} _ =  subst (_≢ 0ᵇ) x'≡x'∸0 2[1+x]≢0
  where
  x'≡x'∸0 = sym (x∸0≡x x')
...
x≥y⇒x≡[x∸y]+y {x'@(2[1+ x ])} {y'@(1+[2 y ])} (inj₁ (odd<even y≤x)) =
  ...
  open ≤-Reasoning
  x∸y = x ∸ y
  eq = let _+_ = _⊕_;  a = var 0F;  b = var (sucF 0F);  c = var (sucF (sucF 0F))
       in
...
+-∸ :  ∀ x {y o} → o ≤ y → (x + y) ∸ o ≡ x + (y ∸ o)
+-∸ x {y} {o} o≤y =  begin-equality
  (x + y) ∸ o                  ≡⟨ sym (fromℕ-toℕ ((x + y) ∸ o)) ⟩
  fromℕ (toℕ ((x + y) ∸ o))    ≡⟨ cong fromℕ (toℕ-homo-∸ (x + y) o) ⟩
  fromℕ (toℕ (x + y) ℕ.∸ n)    ≡⟨ cong (fromℕ ∘ (ℕ._∸ n)) (toℕ-homo-+ x y) ⟩
...
*-distribˡ-∸ :  FP-Bin._DistributesOverˡ_ _*_ _∸_
...
*-distribʳ-∸ :  _*_ FP-Bin.DistributesOverʳ _∸_

...
-- Relation.Nullary.Choose
------------------------------------------------------------------------

{-# OPTIONS --without-K --safe #-}

module Relation.Nullary.Choose.Core where
...
open import Relation.Nullary.Orthogonal using (_⫫[_]_)
open import Relation.Nullary.Choice2 as Choice
...
record Choose (A : Set ℓa) (¬A : Set ℓaⁿ) : Set (ℓa ⊔ ℓaⁿ) where
  constructor _because_
  field
    does  : Bool
    proof : Choice A ¬A does
...
module _ {A : Set ℓa} {P : Set p} {¬A : Set ℓaⁿ} {oA : A ⫫[ P ] ¬A} where

  From-yes : Choose A P ¬A oA → Set a
  From-yes (true  because _) = A
  From-yes (false because _) = ⊤

  From-no : Choose A P ¬A oA → Set a
